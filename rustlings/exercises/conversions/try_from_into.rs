...
const RGB: std::ops::RangeInclusive<i16> = 0..=255;
...
impl TryFrom<[i16; 3]> for Color {
    ...
    fn try_from(arr: [i16; 3]) -> Result<Self, Self::Error> {
        if RGB.contains(&arr[0]) && RGB.contains(&arr[1]) && RGB.contains(&arr[2]) {
...
impl TryFrom<&[i16]> for Color {
    ...
    fn try_from(slice: &[i16]) -> Result<Self, Self::Error> {
        ...
        if RGB.contains(&slice[0]) && RGB.contains(&slice[1]) && RGB.contains(&slice[2]) {
